...
function onSubmit(evt) {
    evt.preventDefault();
    const { delay: delay1 , step , amount  } = evt.currentTarget.elements;
    for(let i = 0, position1 = 1; i < amount.value; i += 1, position1 += 1){
        const delayAll = Number(delay1.value) + step.value * i;
        createPromise(position1, delayAll).then(({ position , delay  })=>{
